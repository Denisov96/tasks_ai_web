...
export async function updateTask(taskData, userId) {
  ...
  try {
    const { data } = await makeRequest(
      `http://localhost:3000/api/users/${userId}/tasks/${taskData.id}`,
      "PUT",
      {
...
export async function deleteTask(userId, options = {}) {
  ...
  if (id) {
    url = `http://localhost:3000/api/users/${userId}/tasks/${id}`;
  } else if (Array.isArray(ids) && ids.length > 0) {
    url = `http://localhost:3000/api/users/${userId}/tasks`;
    config.body = JSON.stringify({ ids: ids.map(Number) });
  } else {
